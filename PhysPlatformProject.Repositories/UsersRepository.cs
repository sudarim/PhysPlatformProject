...
namespace PhysPlatformProject.Repositories
{
   public interface IUsersRepository
    ...
    public class UsersRepository:IUsersRepository
    {
        ...
        {
            List<User> us = db.Users.Where(temp => temp.Email == Email && temp.PasswordHash == PasswordHash).OrderBy(temp => temp.Name).ToList();
            return us;
        }
        public List<User> GetUsersByEmail(string Email)
        {
            List<User> us = db.Users.Where(temp => temp.Email == Email).OrderBy(temp => temp.Name).ToList();
            return us;
        }
        public List<User> GetUsersByUserID(int UserID)
        {
            List<User> us = db.Users.Where(temp => temp.UserID == UserID).OrderBy(temp => temp.Name).ToList();
            return us;
        }
